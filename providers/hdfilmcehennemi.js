...
function requestText(url, referer) {
  var headers = { Accept: "text/html,application/xhtml+xml,application/json;q=0.9,*/*;q=0.8", "User-Agent": USER_AGENT, Referer: "" };
...
function requestJson(url, headers) {
  ...
  return fetch(url, { method: "GET", headers: requestHeaders }).then(function(response) {
    if (!response.ok) throw new Error("HTTP " + response.status + " for " + url);
...
function addMedia(list, value, baseUrl) {
  ...
  if (/\/site\.webm(?:[?#]|$)|\/rekla\/|\/ads?\//i.test(url)) return;
...
function resolveTmdbId(identifier, mediaType) {
  ...
  return requestJson(url, null).then(function(data) {
...
function fetchMetadata(identifier, mediaType) {
  return resolveTmdbId(identifier, mediaType).then(function(tmdbId) {
    ...
    return Promise.all(urls.map(function(url) { return requestJson(url, null).catch(function() { return null; }); })).then(function(dataList) {
      var titles = [];
      var year = null;
...
function collectHdfCandidates(payload, candidates, mediaType) {
  var html = "";
  var results = payload && payload.results;
  if (payload && payload.html) html += String(payload.html);
  if (typeof payload === "string") html += payload;
  if (Array.isArray(results)) {
    results.forEach(function(item) {
      if (typeof item === "string") html += " " + item;
      if (item && typeof item === "object" && item.url) {
        var objectUrl = absoluteUrl(item.url, BASE_URL + "/");
        var objectTitle = cleanProviderTitle(item.title || item.name || item.text);
        if (objectTitle && isHdfCandidateUrl(objectUrl, mediaType) && !candidates.some(function(candidate) { return candidate.url === objectUrl; })) {
          candidates.push({ url: objectUrl, title: objectTitle, text: item.text || item.description || "", searchScore: 75 });
        }
      }
    });
  }
  extractAnchors(html).forEach(function(anchor) {
    ...
    if (!title || !isHdfCandidateUrl(url, mediaType) || candidates.some(function(item) { return item.url === url; })) return;
...
function searchSeries(metadata, mediaType, season) {
  ...
  return Promise.all(searchQueries(metadata, mediaType, season).map(function(query) {
    return requestHdfSearch(query).then(function(payload) { if (payload) collectHdfCandidates(payload, candidates, mediaType); });
  })).then(function() {
...
function findSeasonPage(html, season) {
  ...
  var matches = extractAnchors(html).filter(function(anchor) {
    return /\/dizi\//i.test(anchor.url) && new RegExp("(^|\\D)" + wanted + "\\s*sezon", "i").test(anchor.url + " " + anchor.title + " " + anchor.text);
  });
...
function streamsForCandidate(candidate, metadata, mediaType, season, episode) {
  return requestText(candidate.url, BASE_URL + "/").then(function(detailHtml) {
    ...
    var episodeUrl = findEpisodeLink(detailHtml, season, episode);
...
function getStreams(tmdbId, mediaType, season, episode) {
  ...
  return fetchMetadata(identifier, mediaType)
    ...
    .then(function(result) {
      if (!result.candidate) throw new Error("HDFilmCehennemi eşleşmesi bulunamadı");
